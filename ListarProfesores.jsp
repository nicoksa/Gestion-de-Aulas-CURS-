...
<html lang="en"><head>
    ...
    $(document).ready(function () {
        var table = $('#profesores_table').DataTable({
            "paging": true,
            "pageLength": 5,
            ...
            "initComplete": function () {
                ...
                $('#profesores_table_filter input').attr('placeholder', 'Buscar en todas las columnas...');

                // Cambiar el alto del campo de b�squeda
                $("#profesores_table_filter input").css({
                    'padding': '8px',
                ...
                $(document).on('click', '.custom-clear', function () {
                    $('#profesores_table_filter input').val('').trigger('keyup');
                });
            }
        });
    });
...
<body>
	...
	<jsp:include page="barranavegacion.jsp" />
        ...
        <main b-dk557gbzy5="" role="main" class="w-full px-4 pt-2">
        	<section class="flex min-h-dvh w-full flex-col items-center">
			    <h2 class="w-full bg-gradient-to-tr from-zinc-800 to-zinc-500 p-4 text-center text-4xl font-bold text-gray-50">LISTADO DE PROFESORES</h2><br>
				...
				%>
				    <div style="background-color: #d4edda; color: #155724; padding: 10px; border: 1px solid #c3e6cb; border-radius: 5px; margin-bottom: 15px;">
				        <strong> Profesor dado de baja correctamente</strong>
				    </div>
				<%
				...
				%>
				    <div style="background-color: #f8d7da; color: #721c24; padding: 10px; border: 1px solid #f5c6cb; border-radius: 5px; margin-bottom: 15px;">
				        <strong>Error: No se pudo dar de baja el profesor - Verifique la existencia de reservas activas</strong> 
				    </div>
				<%
				    }
				%>
				...
				%>
				    <div style="background-color: #d4edda; color: #155724; padding: 10px; border: 1px solid #c3e6cb; border-radius: 5px; margin-bottom: 15px;">
				        <strong> Datos editados correctamente.</strong>
				    </div>
				<%
				...
				<table id="profesores_table" class="display">
				    <thead>
				        <tr>
				            ...
				            <th class="border-b border-slate-500 bg-zinc-700 p-4 text-sm font-normal leading-none text-slate-50"style="width: 35%;">
				                Email
				            </th>
				            <th class="border-b border-slate-500 bg-zinc-700 p-4 text-sm font-normal leading-none text-slate-50"style="width: 10%;">
				                Telefono
				            </th>
				            <th class="border-b border-slate-500 bg-zinc-700 p-4 text-sm font-normal leading-none text-slate-50"style="width: 20%;">
				                Acciones
				    ...
				    <tbody>
				        <% for (Profesor profesor : listaProf) { %>
    ...
    <a class="mt-0 w-36 h-11 rounded-md border border-transparent bg-gradient-to-tr from-orange-900 to-pink-500 px-4 py-2 gap-2 text-center text-l text-white shadow-md transition-all hover:shadow-lg hover:bg-slate-700 focus:bg-slate-700 focus:shadow-none active:bg-slate-700 active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none flex items-center justify-center mr-2"
				                       href="ServletProfesores?dniParaEliminar=<%= profesor.getDni() %>">
    ...
    <a class="mt-0 w-36 h-11 rounded-md border border-transparent bg-gradient-to-tr from-sky-800 to-sky-500 px-4 py-2 gap-2 text-center text-l text-white shadow-md transition-all hover:shadow-lg hover:bg-slate-700 focus:bg-slate-700 focus:shadow-none active:bg-slate-700 active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none flex items-center justify-center"
       href="ServletProfesores?dniParaEditar=<%= profesor.getDni() %>">
    ...
    <a class="w-64 h-11 rounded-md border border-transparent bg-gradient-to-tr from-sky-800 to-sky-500 px-4 py-2 gap-2 text-center text-l text-white shadow-md transition-all hover:shadow-lg hover:bg-slate-700 focus:bg-slate-700 focus:shadow-none active:bg-slate-700 active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none flex items-center justify-center" 
       href="ServletProfesores?Param=listinactivos">
